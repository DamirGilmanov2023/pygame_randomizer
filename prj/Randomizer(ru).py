...
d=input("Введите количество участриков=")
d=int(d)
name=[]
n=0
while n<d:
    name.append(input(f"Введите имя участника №{n+1}:"))
    n+=1
pygame.init()
...
while RUN:
    ...
    text_start = f.render("Крутить", True, WHITE)
    ...
    if v-10<0:
        v=10
    text_v = f.render(f'Скорость={round(FPS-10,4)}', True, (255, 255, 255))
    screen.blit(text_v, (590, 60))
    text_tr = f.render(f'Трение={round(tr,4)}', True, (255, 255, 255))
